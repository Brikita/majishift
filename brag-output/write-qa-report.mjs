...
const report=JSON.parse(fs.readFileSync('brag-output/evidence/package-verification.json','utf8'));
fs.writeFileSync('brag-output/QA-REPORT.md',`# MajiShift demo QA

## Verified output
- Duration: ${report.duration} seconds; ${report.width} × ${report.height}; H.264, ${report.frameRate} fps.
...
## Integrity
MP4 SHA-256: ${report.sha256}

## Visual review
Final encoded frames and the recorded interaction sequence must be reviewed before delivery; record the result below.
`);
console.log('Wrote media and source QA report.');
