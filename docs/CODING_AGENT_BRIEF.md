...
The app is React/TypeScript using Vinext/Vite and Sites-compatible hosting. lib/planner.ts is the deterministic engine; app/page.tsx is the current client UI. All defaults and rain are synthetic. No live Conduit import, GIS, trained Adaption inference or persistent records exist. The numerical heuristic is not a proven optimizer or field-validated system.
...
Implement work packages in dependency order. Prioritize correct topology, data readiness, meaningful Conduit use, full-horizon planning, fair baseline comparison and receipts. Add the map using the same calculated results. Do not invent asset coordinates, pipe routes, bathymetry, building heights, service effects, forecast history or savings. Provide an accessible map fallback.
