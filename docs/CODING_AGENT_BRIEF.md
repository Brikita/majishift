...
The app is React/TypeScript using Vinext/Vite and Sites-compatible hosting. `lib/planner.ts` is the deterministic engine; `app/page.tsx` is the client UI. The latest public Conduit station 61 observation is normalized server-side and its two cumulative gauge totals feed Day 1 direct-rain accounting. Days 2–7 and all reservoir/operator inputs are explicitly synthetic scenarios. A responsive MapLibre campus atlas is implemented. Two 30B AutoScientist runs finished at 0.5371 and 0.5567 against a 0.80 target. A corrected Gemma 4B run is training on a fused prompt/context dataset with much less augmentation. No checkpoint is served or evaluated on the 20 untouched project cases. The numerical heuristic is not a proven optimizer or field-validated system.
