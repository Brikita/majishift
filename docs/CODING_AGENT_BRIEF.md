...
The app is React/TypeScript using Vinext/Vite and Sites-compatible hosting. `lib/planner.ts` is the deterministic engine; `app/page.tsx` is the client UI. The default historical replay uses the official Conduit station 61 CSV: daily rain enters the water balance and SHT temperature ranges generate FAO-56 Hargreaves reference evapotranspiration. The app shows the resulting change against the fixed evaporation assumption. The latest public observation is normalized separately when available. Reservoir/operator inputs remain explicitly synthetic scenarios. A responsive MapLibre campus atlas is implemented. Three AutoScientist runs finished: strict 30B at 0.5371, enhanced-label 30B at 0.5567 and fused-context Gemma 4B at 0.5345, all against a 0.80 target. No checkpoint is served or evaluated on the 20 untouched project cases. The numerical heuristic is not a proven optimizer or field-validated system.
