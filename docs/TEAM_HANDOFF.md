...
| Weather | Latest public Conduit station 61 observation is fetched server-side; timestamp, coordinates, raw cumulative gauge totals, temperature, humidity and wind are shown with provenance | Confirm gauge semantics and station representativeness with the data owner; obtain history before calculating rainfall increments; keep planning rainfall separate from observations unless an authorized forecast is added |
| AI | Three runs completed: strict 30B 0.5371, enhanced-label 30B 0.5567 and fused-context Gemma 4B 0.5345; all missed the 0.80 target; 20 cases remain held out | Archive-verify the enhanced checkpoint; evaluate checkpoints on untouched cases; select honestly; securely serve only a validated choice |
| Demo records | 1,260 relative-day records generated by the planner; every row says `evidence_status=synthetic` and `operator_review_status=not_reviewed` | With approval, replace them with operator records for October; preserve source IDs, units, timestamps and review history |
...
- `lib/planner.ts`: Config, defaults, validate, simulate, compare. All operating defaults and the seven-day rainfall array are invented scenario inputs. Conduit observations are kept as environmental context until a valid incremental rainfall series is available. `simulate` prioritizes essential use and conserves aggregate water.
