...
| Weather | Official Conduit station 61 CSV drives measured daily rain and temperature-derived reference evapotranspiration in a seven-day replay; the latest observation is fetched separately when available | Confirm station representativeness and the Hargreaves/open-water relationship; add an authorized forecast before prospective planning |
...
- `lib/planner.ts`: Config, defaults, validate, simulate, compare. Official Conduit daily rain and reference evapotranspiration drive the historical replay; all reservoir operating defaults remain invented scenario inputs. `simulate` prioritizes essential use and conserves aggregate water.
